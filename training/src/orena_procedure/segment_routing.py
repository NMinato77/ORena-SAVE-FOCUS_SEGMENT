"""Deterministic semantic and sampling routes for SEGMENT question analysis.

The parser is an analysis aid, not an oracle.  It deliberately consumes only
the question, the official answer-format name, and (when available) the
official primary capability.  It never reads the answer or any video-derived
annotation.  The routing hierarchy is:

``official metadata -> answer format -> question wording -> conservative fallback``

The output is intentionally coarse enough to support aggregate analysis while
retaining fine-grained labels for auditing borderline cases.
"""
...
TIME_SUBTYPES = frozenset({"duration", "single_point", "multi_point", "unknown"})
...
_STATIC_PRIMARY = frozenset(
    {
        "object_attributes",
        "spatial_localization_camera",
        "spatial_localization_situs",
    }
)
...
_AGGREGATION_PRIMARY = frozenset(
    {"object_aggregation", "event_aggregation", "duration_estimation"}
)
_STATIC_D_HIRES_FINE = frozenset({"object_attributes", "object_identity_presence"})

_ACTUAL_TIME_RE = re.compile(r"\b\d{1,2}:\d{2}:\d{2}\b")
_TIME_RE = re.compile(r"\b(?:\d{1,2}:\d{2}:\d{2}|hh:mm:ss)\b", re.I)
...
class Routing:
    """All deterministic routing fields emitted for one question."""
...
def extract_objects(question: str) -> tuple[str, ...]:
    """Return explicit official foreign-object classes in question order."""
...
def _has(question: str, pattern: str) -> bool:
    return bool(re.search(pattern, str(question), re.I))
...
def _infer_answer_format(question: str) -> str:
    """Infer the likely output format from wording only."""

    q = str(question)
    # The option clause is authoritative for question-only routing.  It must
    # precede timestamp detection because many spatial choices mention an
    # explicit timepoint.
    if _has(q, r"\bPlease\s+select\s+(?:none\s*,\s*)?(?:one\s+answer|one\s+or\s+multiple\s+answers)\s*:"):
        return "multiple_choice"
    if _has(q, r"\bin %\b|\bpercent(?:age)?\b|%"):
        return "percentage"
    if _has(
        q,
        r"hh:mm:ss|\bat what time\b|\bat which time\b|\bwhat time\b|"
        r"\btimepoint\b|\btime points?\b",
    ):
        return "time"
    if _has(q, r"select (?:one|none, one or multiple)|multiple answers|quadrant"):
        return "multiple_choice"
...
def infer_answer_format(question: str) -> str:
    """Infer the output format from question wording only.

    The result is a deployment hint, not an official annotation lookup.  In
    particular, an ``open_ended`` question may still request a constrained
    answer such as yes/no or ``hh:mm:ss`` in its wording.
    """

    q = str(question)
    # This is deliberately stricter than the legacy routing heuristic.  A
    # timestamp used as an anchor (for example, "at timepoint 00:40:39") is
    # not evidence that the answer itself should be a timestamp.
    if _has(q, r"\bPlease\s+select\s+(?:none\s*,\s*)?(?:one\s+answer|one\s+or\s+multiple\s+answers)\s*:"):
        return "multiple_choice"
    ...
    if _has(
        q,
        r"hh:mm:ss|\bat what time\b|\bat which time\b|\bwhat time\b|"
        r"\btimestamp\b|\btime points?\b|"
        r"\bwhen (?:was|were|did) .*\b(?:inserted|retrieved|removed|visible|appeared|occurred)\b",
    ):
        return "time"
...
def infer_time_subtype(question: str) -> str:
    """Infer the time answer semantics from question wording only.

    The official dataset exposes all of these cases as ``time``.  This
    internal subtype is therefore used only to make the prompt and diagnostics
    precise; it must never replace the official answer format during offline
    scoring.
    """

    q = str(question)
    # Duration has precedence because duration questions often mention several
    # intervals and still request a single HH:MM:SS value.
    if _has(
        q,
    ...
    if _has(
        q,
        r"\btime points\b|\btimepoints\b|\beach individual\b|\bmultiple timestamps?\b|"
        r"\bfrom when to when\b|\bfirst and last\b|\bstart and end\b",
    ):
        return "multi_point"
    if _has(
        q,
        r"\bat what time\b|\bwhat time\b|\bwhen (?:was|were|did|does|is|are)\b|"
        r"\btimestamp\b|\btime point\b",
    ):
        return "single_point"
    return "unknown"
...
def _infer_primary(question: str, answer_format: str) -> str:
    """Infer a conservative capability-like label from question wording."""
...
def _is_explicit_restriction(question: str) -> bool:
    ...
    return bool(
        ...
        or _has(
            q,
            r"\bbefore\b|\bafter\b|\bbetween\b|\bwhile\b|"
            r"\buntil\b|\bfrom\b.+\bto\b|at timepoint|at time point",
        )
    )
...
def _fine_route(question: str, answer_format: str, primary: str, event: str, aggregation: str) -> str:
    ...
    if primary == "object_identification" and event in {
        "cooccurrence",
        "manipulation",
    }:
        return "event_occurrence"
...
def _medium_route(fine: str) -> str:
    ...
    if fine == "event_occurrence" or fine == "event_localization" or fine == "event_object_identity":
...
def _scope_and_sampling(
    ...
    global_wording = _has(
        q,
        r"whole video|throughout the video|all time intervals|longest.*duration|"
        r"classes appear|what types.*seen|for most of the time|"
        r"populated with|longest continuous time",
    )
...
def route_question(question: str, answer_format: str = "", primary: str = "") -> Routing:
    """Return a deterministic route using no answer-bearing fields."""

    q = str(question)
    supplied_format = bool(answer_format)
    supplied_primary = bool(primary)
    fmt = str(answer_format) if supplied_format else infer_answer_format(q)
    capability = str(primary) if supplied_primary else _infer_primary(q, fmt)
    objects = extract_objects(q)
    event = _event_type(q)
    relation = _temporal_relation(q)
    aggregation = _aggregation_type(q, fmt, capability)
    fine = _fine_route(q, fmt, capability, event, aggregation)
    medium = _medium_route(fine)

    if medium in {"clinical_reasoning"}:
    ...
    scope, sampling, scope_note = _scope_and_sampling(q, fmt, capability, medium, fine)
    if supplied_primary or supplied_format:
        basis = "official_primary+answer_format+wording" if supplied_primary else "answer_format+wording"
    else:
        basis = "question_wording"
    known_primary = bool(capability)
    strong_rule = fine != "general_other" and (known_primary or fmt in {"time", "number", "percentage"})
    confidence = "high" if strong_rule and fine != "clinical_reasoning" else "medium"
    if fine == "general_other" or (not capability and not fmt):
        confidence = "low"
    notes = scope_note
    if len(objects) > 2:
        notes += "; multiple explicit FO classes"
    if capability in _COMPLEX_PRIMARY:
